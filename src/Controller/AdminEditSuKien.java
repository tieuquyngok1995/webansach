 ...
 */
@WebServlet("/AdminEditTinTuc")
public class AdminEditSuKien extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public AdminEditSuKien() {
		super();
	...
	private void toDo(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (error) {
			...
			try {
				if (TinTucDAO.updateTieuDe(c)&&TinTucDAO.updatenDTT(c)
							&&TinTucDAO.updatenDC(c)&&TinTucDAO.updateNgayDang(c)	
				) {
